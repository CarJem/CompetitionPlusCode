namespace CompPlus_Status
{
    bool ReplaceSpecialRings = true;
    bool SpecialRingsNeedRefresh = true;
    bool isVSControllerInputUnlocked = false;
    int CurrentLevelSelect = 0;
    bool DisablePlayerDying = false;
    bool DisableDynCam = false;
    bool EnableHUBSpecialRingFunctionality = false;
    bool InHolloweenHUB = false;
    bool UseEXETitleCard = false;
    bool DisableWorldPositionWrite = false;
    bool ForceLoadChibiSprites = false;
    bool IsExecutorStage = false;
    bool IsChaotixStage = false;
    bool IsChaotixStageEncore = false;

    bool Abilities_CanDynamicallyChangeEffects = true;
    bool Abilities_EnableSkins = true;
}